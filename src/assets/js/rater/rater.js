...
$("#demography_new_gmc").on("submit", function(event) {
    ...
    if ($("#demography_new_gmc").parsley().isValid()) {
        $.ajax({
            url: baseURL + "/rater/store-demography",
            method: "POST",
            data: formData,
            dataType: "json",
            processData: false,
            ...
            success: function(data) {
                if (data.status == 'true') {
                        window.location.assign(baseURL + "/rater/quote/group-health-insurance-policy");
                } else {
                    swal(data.message);
                    return false;
                }
...
$("#demography_gpa_new").on("submit", function(event) {
    ...
    if ($("#demography_gpa_new").parsley().isValid()) {
        $.ajax({
            url: baseURL + "/rater/demography/storeGpaDemography",
            method: "POST",
            data: formData,
            dataType: "json",
            processData: false,
            ...
            success: function(data) {
                if (data.status == 'true') {
                    window.location.assign(baseURL + "/rater/quote/group-personal-accident-policy");
                } else {
                    swal({
                        title: "",
                        text: data.message,
...
const gmcPolicyType = (type) =>{
    console.log(type);
    if(type == 1){
        $(".claim_gmc_hide_show").show();
    }else{
        $(".claim_gmc_hide_show").hide();
    }
}